...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    document.getElementById('advisorNameDisplay').textContent = user.name || user.email;
...
function displayApplications() {
    ...
    if (currentFilter === 'pending') {
        filteredApps = allApplications.filter(app => app.status === 'Pending');
    } else if (currentFilter === 'approved') {
        filteredApps = allApplications.filter(app => 
            app.status === 'Advisor Approved' || 
            app.status === 'HOD Approved' || 
            app.status === 'Approved'
        );
    } else if (currentFilter === 'rejected') {
        filteredApps = allApplications.filter(app => app.status === 'Rejected');
    }
    ...
    if (filteredApps.length === 0) {
        container.innerHTML = `
            <div class="empty-state" style="text-align: center; padding: 3rem;">
                ...
                <p style="color: #999;">No ${currentFilter} applications at the moment.</p>
            </div>
        `;
        return;
    }

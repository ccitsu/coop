...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (!user || user.userType !== 'vicedean') {
        window.location.href = 'vicedean-login.html';
        return;
    }
    
    document.getElementById('vicedeanNameDisplay').textContent = user.name || user.email;
...
function displayApplications() {
    ...
    if (currentFilter === 'pending') {
        filteredApps = allApplications.filter(app => app.status === 'HOD Approved');
    } else if (currentFilter === 'approved') {
        filteredApps = allApplications.filter(app => app.status === 'Approved');
    } else if (currentFilter === 'rejected') {
        filteredApps = allApplications.filter(app => app.status === 'Rejected' && app.rejectorType === 'vicedean');
    }
    ...
    if (filteredApps.length === 0) {
        container.innerHTML = `
            <div class="empty-state" style="text-align: center; padding: 3rem;">
                ...
                <p style="color: #999;">No ${currentFilter} applications available.</p>
            </div>
        `;
        return;
    }
        ...
        appCard.innerHTML = `
            ...
            <div class="application-details-grid">
                ...
                <div class="detail-item">
                    ...
                    <span>${app.hodApprovalDate ? formatDate(app.hodApprovalDate) : 'N/A'}</span>
                </div>
            </div>
    ...
    detailsContainer.innerHTML = `
        ...
        <div style="margin-top: 20px;">
            <div style="padding: 10px; background: #d4edda; border-radius: 5px; margin-bottom: 10px;">
                <strong style="color: #155724;">✓ Approved by Academic Advisor</strong><br>
                <small>${app.advisorApprovalDate ? formatDate(app.advisorApprovalDate) : 'N/A'}</small>
            </div>
            <div style="padding: 10px; background: #cce5ff; border-radius: 5px;">
                <strong style="color: #004085;">✓ Approved by Head of Department</strong><br>
                <small>${app.hodApprovalDate ? formatDate(app.hodApprovalDate) : 'N/A'}</small>
            </div>
        </div>
    `;
...
function setupModal() {
    ...
    approveBtn.addEventListener('click', async function() {
        if (!confirm('Are you sure you want to give final approval? This will generate and send the leave document to all parties.')) return;
        
        try {
            const user = JSON.parse(localStorage.getItem('currentUser'));
            const response = await makeAPICall(CONFIG.ENDPOINTS.APPROVE_APPLICATION, {
                applicationId: currentApplicationId,
                approverType: 'vicedean',
                approverName: user.name || user.email,
                approvalDate: new Date().toISOString()
            });
            
            if (response.success) {
                alert('Application finally approved! Leave document has been generated and sent to all parties.');
                closeModal();
                await loadApplications();
            } else {
